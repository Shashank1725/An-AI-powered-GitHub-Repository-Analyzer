...
GITHUB_TOKEN=os.getenv("GITHUB_TOKEN",st.secrets['GITHUB_TOKEN'])
...
def generate_response(prompt):
    ...
    for chunk in stream:
        for char in chunk.text:
            time.sleep(0.01)
            yield char
st.set_page_config(page_title="Repo Explainer",page_icon="🤖")
...
if st.session_state.repo is None:
    ...
    model=genai.GenerativeModel("gemini-2.5-flash",
                                system_instruction="You are coding expert who analyses Github repos."
                                "When replying, be succint and polite .Avoid markdown title headers. "
                                "When citing file or variables, use backticks for markdown formatting."
                                f"Base your answers on this repos :{st.session_state.repo}")
